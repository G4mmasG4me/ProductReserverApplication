...
if __name__ == '__main__':
  while running is True:
    ...
    mycursor.execute('SELECT DISTINCT order_item.product_id, product_link.id, product_link.link, product_link.last_availability, address.country FROM unfilled_order_item INNER JOIN order_item ON unfilled_order_item.order_item_id = order_item.id INNER JOIN address ON order_item.address_id = address.id INNER JOIN product_link WHERE (product_link.product_id = order_item.product_id AND product_link.region = address.country)')
    ...
    for availability_position, availability in enumerate(availability_output):
      ...
      if availability is True:
        ...
        # get a list of all the data for that particular product
        ...
        product_details = tuple(zip(*zip(*product_details), product_link)) # adds extra column to 2d product list

        for email in emails:
          p = multiprocessing.Process(target=email_customer.send_stock_alert, args=(email, product_details))
...
# check if stock has changed since last tmie
# if has changed to true then send email
# if has changed to false then don't do anything
# if hasn't changed then don't do anything